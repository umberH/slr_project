from langchain.llms import HuggingFaceHub  # Import the HuggingFaceHub class for language model interactions
...
os.environ["HUGGINGFACEHUB_API_TOKEN"] = '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'
# HUGGINGFACEHUB_API_TOKEN = getpass()

# Load environment variables, particularly the HuggingFaceHub API key
# load_dotenv()
coding_problem = ""

def generate_DS_problem(topic, difficulty):
    repo_id = "01-ai/Yi-34B"
    # Initialize the HuggingFaceHub language model with a specific temperature setting
    # llm = OpenAI(temperature=0.7)
    llm = HuggingFaceHub(
    repo_id=repo_id, model_kwargs={"max_length": 128, "temperature": 0.7}
    )
